...
async def get_users(
    *,
    username: Annotated[
        str | None, Query(description='A username or no argument for all users')
    ] = None,
    db: DbSession,
) -> User | list[User]:
    if username:
...
async def get_user_by_id(
    user_id: Annotated[int, Path(description='A user ID')], db: DbSession
) -> User:
...
async def update_current_user(
    user_update: UserUpdate, current_user: CurrentUser, db: DbSession
) -> User:
